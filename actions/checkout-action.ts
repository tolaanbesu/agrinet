...
export async function placeOrderAction(data: { phone: string, address: string, paymentMethod: string, orderId?: string }) {
    ...
    try {
        ...
        if (data.orderId) {
            ...
            cart = { items: order.items.map(item => ({ ...item, product: item.product })) }
        } else {
            // Regular cart processing
